...
use super::visual_queue::{request_thumbnail, VisualState};
...
type IconSource = ArcRwSignal<VisualState>;
...
impl IconSourceCache {
    ...
    fn get_or_insert(&mut self, key: String) -> (IconSource, bool, u64) {
        ...
        if self.entries.contains_key(&key) {
            ...
            let (source, should_load, generation) = {
                ...
                let should_load = !entry.loading
                    && entry.source.get_untracked() == VisualState::Missing
                    && retry_due;
                if should_load {
                    entry.loading = true;
                    entry.generation = retry_generation;
                    entry.retry_after_ms = None;
                    entry.source.set(VisualState::Loading);
                }
        ...
        let source = ArcRwSignal::new(VisualState::Loading);
        self.entries.insert(
            key.clone(),
            IconCacheEntry {
                source: source.clone(),
...
fn thumbnail_pixel_size(visual_size: u32) -> u32 {
    let pixel_ratio = web_sys::window().map_or(1.0, |window| window.device_pixel_ratio());
    super::numeric::ceil_to_u32(f64::from(visual_size) * pixel_ratio).clamp(32, 256)
}
        ...
        spawn_local(async move {
            ...
            for attempt in 0..ICON_LOAD_ATTEMPTS {
                match backend::visual(&path, 64, false).await {
                    Ok(Some(icon)) => {
                        source_for_load.set(VisualState::Ready(icon));
            ...
            source_for_load.set(VisualState::Missing);
            finish_icon_load(
                &cache_key,
                generation,
    ...
    view! {
        <span class="file-icon grid size-5 place-items-center [&>img]:size-5 [&>img]:object-contain">
            {move || match source.get() {
                VisualState::Ready(source) => view! {
                    <img src=source alt="" loading="eager" decoding="async" />
                }.into_any(),
                VisualState::Loading | VisualState::Missing => fallback_icon(is_dir),
            }}
        </span>
    }
}
...
pub(crate) fn FileVisual(
    ...
    let should_request_file_thumbnail = load && !is_dir;
    let subscription = should_request_file_thumbnail.then(|| {
        request_thumbnail(
            path.clone(),
    ...
    if let Some(subscription) = subscription {
        ...
        view! {
            <span class="icon-result-visual thumbnail-stack grid size-[var(--view-icon-size)] shrink-0 place-items-center [&>*]:[grid-area:1/1]">
                {move || (!matches!(fallback_source.get(), VisualState::Ready(_))).then(|| view! {
                    <FileIcon path=path.clone() is_dir />
                })}
                {move || match source.get() {
                    VisualState::Ready(source) => Some(view! {
                        <img
                    ...
                    VisualState::Loading | VisualState::Missing => None,
                }}
            </span>
        }
        .into_any()
