use super::numeric::{ceil_to_u32, floor_to_u32, trunc_to_i32};
use super::view_modes::{ViewMode, GRID_GAP, GRID_PADDING};
use crate::app::search_workspace::search::VIRTUALIZATION_OVERSCAN;
...
impl ResultViewport {
    ...
    pub fn visible_start(self) -> Memo<u32> {
        Memo::new(move |_| {
            ...
            let first_row = floor_to_u32(self.scroll_top.get() / mode.item_height());
            first_row
    ...
    pub fn visible_end(self, start: Memo<u32>, total: RwSignal<u32>) -> Memo<u32> {
        Memo::new(move |_| {
            let mode = self.mode.get();
            let columns = self.columns.get();
            let visible_rows = ceil_to_u32(self.height.get() / mode.item_height());
    ...
    pub fn scroll_row_into_view(self, index: u32) {
        ...
        let top = if mode.is_grid() {
            ...
            f64::from(index) * mode.item_height()
        };
    ...
    pub fn set_mode(self, mode: ViewMode) {
        ...
        self.mode.set(mode);
        let width = self.list_ref.get().map_or_else(
            || self.grid_width.get_untracked(),
            |list| f64::from(list.client_width()),
        );
        ...
        let target = if mode.is_grid() {
            ...
            f64::from(anchor) * mode.item_height()
        };
    ...
    pub fn canvas_height(self, total: u32) -> f64 {
        let mode = self.mode.get();
        if !mode.is_grid() {
            return f64::from(total) * mode.item_height();
        }
    ...
    pub fn page_step(self) -> i32 {
        let mode = self.mode.get_untracked();
        let rows = floor_to_u32(self.height.get_untracked() / mode.item_height()).max(1);
        let columns = self.columns.get_untracked().max(1);
        i32::try_from(rows.saturating_mul(columns)).unwrap_or(i32::MAX)
    }

    pub fn navigation_delta(self, key: &str) -> Option<i32> {
        let columns = i32::try_from(self.columns.get_untracked().max(1)).unwrap_or(i32::MAX);
    ...
    fn first_visible_index(self) -> u32 {
        let mode = self.mode.get_untracked();
        let first_row = floor_to_u32(self.scroll_top.get_untracked() / mode.item_height());
        if mode.is_grid() {
            first_row.saturating_mul(self.columns.get_untracked().max(1))
        } else {
            first_row
        }
    }
    ...
    fn set_scroll_top(self, top: f64) {
        let top = top.max(0.0);
        if let Some(list) = self.list_ref.get() {
            list.set_scroll_top(trunc_to_i32(top));
...
fn calculate_columns(mode: ViewMode, width: f64) -> u32 {
    ...
    floor_to_u32((available + GRID_GAP) / (mode.min_width() + GRID_GAP))
