use super::numeric::trunc_to_i32;
...
fn keyboard_position(index: u32, viewport: ResultViewport) -> (i32, i32) {
    ...
    if !mode.is_grid() {
        ...
        let row_y = rect.top() + f64::from(index) * item_height - f64::from(list.scroll_top())
            + item_height;
        return clamp_to_viewport(trunc_to_i32(rect.left() + 180.0), trunc_to_i32(row_y));
    }
    ...
    clamp_to_viewport(trunc_to_i32(x), trunc_to_i32(y))
}

fn clamp_to_viewport(x: i32, y: i32) -> (i32, i32) {
    let (width, height) = web_sys::window().map_or((1280, 720), |window| {
...
fn viewport_dimension(
    ...
    match result {
        Ok(value) => trunc_to_i32(value.as_f64().unwrap_or_else(|| {
