...
impl ThumbnailCache {
    ...
    fn get(&mut self, key: &ThumbnailKey) -> Option<Option<String>> {
        let source = self.entries.get(key).map(|entry| entry.source.clone());
        if source.is_some() {
    ...
    fn insert(&mut self, key: ThumbnailKey, source: Option<String>) {
        ...
        self.entries
            .insert(key.clone(), CachedThumbnail { source, bytes });
...
impl ThumbnailPipeline {
    ...
    fn complete(&mut self, request: QueuedThumbnail, source: Option<String>) {
        ...
        if request.resource.subscribers.get() > 0 {
            request.resource.source.set(Some(source));
        }
    ...
    fn fail(&mut self, request: QueuedThumbnail) {
        if request.resource.subscribers.get() > 0 {
            request.resource.source.set(Some(None));
        }
        self.remove_resource(&request.key, &request.resource);
    }
...
async fn process_thumbnail_queue() {
    loop {
        ...
        match backend::visual(&request.key.path, request.key.pixel_size, true).await {
            Ok(source) => PIPELINE.with(|pipeline| pipeline.borrow_mut().complete(request, source)),
            Err(error) => {
...
mod tests {
    ...
    fn subscriptions_share_one_pending_request() {
        Owner::new().with(|| {
            ...
            first.source.set(Some(Some("thumbnail".into())));
            assert_eq!(
