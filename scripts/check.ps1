...
$repositoryUrl = "https://github.com/StephanOrgiazzi/EverythingNext"
$PSNativeCommandUseErrorActionPreference = $false
foreach ($legacyIdentifier in $legacyIdentifiers) {
  $matches = @(& git.exe grep -n -F -- $legacyIdentifier 2>$null) |
    Where-Object { -not $_.Contains($repositoryUrl) }
...
cargo test -p everything-engine --locked
cargo test -p windows-shell --locked
cargo check -p everything-core --locked
cargo check -p everything-engine --locked
