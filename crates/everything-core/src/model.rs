...
impl SelectionRange {
    ...
    #[must_use]
    pub fn inclusive_len(self) -> u64 {
        u64::from(self.end.abs_diff(self.start)) + 1
    }
}
...
impl IndexSelection {
    ...
    pub fn count(&self) -> u64 {
        self.ranges
            .iter()
            .copied()
            .map(SelectionRange::inclusive_len)
            .sum()
    }
...
mod tests {
    ...
    fn selection_ranges_are_normalized() {
        ...
        assert_eq!(range.inclusive_len(), 9);
    }

    #[test]
    fn inclusive_length_is_safe_for_deserialized_ordering() {
        let range = SelectionRange { start: 12, end: 4 };
        assert_eq!(range.inclusive_len(), 9);
    }
}
