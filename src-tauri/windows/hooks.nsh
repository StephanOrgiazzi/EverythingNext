...
bundled_engine_upgrade_found:
  DetailPrint "Stopping the existing Everything Next engine..."
  nsExec::ExecToLog '"$0" -instance "EverythingNext" -quit'
  Pop $1
  Sleep 500
  ...
  nsExec::ExecToLog '"$0" -instance "EverythingNext" -uninstall-service'
  Pop $1
  Sleep 500
...
bundled_engine_install_found:
  DetailPrint "Installing the private Everything Next indexing service..."
  nsExec::ExecToLog '"$0" -instance "EverythingNext" -install-service -install-service-pipe-name "\\.\PIPE\Everything Service (EverythingNext)"'
  Pop $1
  StrCmp $1 "0" bundled_service_install_done 0

  MessageBox MB_ICONSTOP|MB_OK "The Everything Next search service could not be installed (exit code $1). Installation will stop." /SD IDOK
  Abort
...
bundled_engine_uninstall_found:
  DetailPrint "Stopping the private Everything Next engine..."
  nsExec::ExecToLog '"$0" -instance "EverythingNext" -quit'
  Pop $1
  Sleep 500

  DetailPrint "Removing the private Everything Next indexing service..."
  nsExec::ExecToLog '"$0" -instance "EverythingNext" -uninstall-service'
  Pop $1
  StrCmp $1 "0" bundled_service_uninstall_done 0

  MessageBox MB_ICONSTOP|MB_OK "The Everything Next search service could not be removed (exit code $1). Uninstallation will stop to avoid leaving an orphaned service." /SD IDOK
  Abort
...
!macro NSIS_HOOK_POSTUNINSTALL
  DeleteRegValue HKCU "Software\Microsoft\Windows\CurrentVersion\Run" "Everything Next"
!macroend
